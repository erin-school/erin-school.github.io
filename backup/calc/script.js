...
function MainWork() {
  ...
  if (isValidInput) {
    ...
    resulttag.html("<h2>"+(probvalue*100).toString()+"%확률 뽑기 "+trials.toString()+"회 시도시 목표 달성률</h2>");
    resulttag.append(successtable);

    // display first few winnings
    resulttag.append("<h2>"+(probvalue*100).toString()+"%확률 뽑기 "+trials.toString()+"회 시도시 N회 이상 당첨 확률(당첨 10회까지)</h2>");
...
function calc_probs_partial(probvalue, trials, numwant) {
  ...
  for (let k =0; k <= max_k; k++) {
    problist[k] = binomial(trials,k)*Math.exp( k*Math.log(probvalue)+(trials-k)*Math.log(q));
  }
  return problist
}
...
function buildFirstFewWinningsTable(problist,trials,numwant) {
  ...
  let tb = probTableTemplate("당첨 횟수","당첨 확률","해당횟수 이상 당첨 확률");
...
function buildRequiredTrialsTable(winningProbabilityPerTrial,targetWinnings,costPerTrial,costUnit,targetProbPercentList) {
  ...
  for (const targetProb of targetProbList) {
    requiredTrialsArray.push(requiredTrials(winningProbabilityPerTrial,targetWinnings,targetProb));
  }
  
  //build table
  let tb = probTableTemplate("달성 가능성(운 상위%)","요구 뽑기 수","예상 비용");
  
  for (let k=0; k<requiredTrialsArray.length; k++) {
    var tr = tb.insertRow();
    let targetProb = targetProbPercentList[k];
    switch (targetProb) {
      case 50: tr.className = "table-info"; break;
      case 90: tr.className = "table-success"; break;
      case 95: tr.className = "table-warning"; break;
      case 98: tr.className = "table-primary"; break;
      case 99: tr.className = "table-danger"; break;
      default: break;
    }
