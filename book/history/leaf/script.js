...
theater
  .addActor('tell1', { speed: 0.8,accuracy: 1 })
    ...
    .addScene('tell1:사실상 늑대의 켈이나 다름 없는 이리시스였지만 스콜라스의 함대를 이끄는 피키스의 공세로', 850)
    .addScene('tell1:소행성 에오스로 몰리게 된다.에오스 격돌로 알려진 이 전투는 스콜라스의 승리로 끝났고', 850)
    .addScene('tell1:이리시스와 그녀의 함대는 전멸하게 된다', 850)
    ...
    .addScene('tell1:파리사스마저 처치한 스콜라스는 켈의 자리를 확보하게 되고 늑대의 켈로서', 850)
    .addScene('tell1:자신들을 학살한 각성자들한테 본때를 보여주기로 결심한다.', 850)