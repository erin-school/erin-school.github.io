...
theater
  .addActor('tell1', { speed: 0.8,accuracy: 1 })
    ...
    .addScene('tell1:팔라스 공성전', 850)
    .addScene('tell1:스콜라스는 리프가 이리스 전투에서 큰 피해를 입었다는 걸 알고 있었고', 850)
    .addScene('tell1:공세를 가하면 리프가 반격하지 못 할거라는 것 또한 알고 있었다', 850)
    .addScene('tell1:결국 스콜라스는 리프의 최대 정착지인 소행성 히기에이아를 공격하게 되고', 850)
    .addScene('tell1:리프는 처참한 패배를 맞이하게 된다.', 850)
    .addScene('tell1:자수정 기지에 이어 히기에이아 마저 잃은 리프는 점점 열세에 빠지게 되지만', 850)
    .addScene('tell1:울드렌이 늑대들의 암호를 뚫는데 성공한다.', 850)
    .addScene('tell1:늑대들의 통신을 보던 중 울드렌과 휘하의 대원들은 ', 850)
    .addScene('tell1:늑대들이 소행성 밤베르가의 이심률을 잘못 계산했다는 걸 알아챈다.', 850)
    .addScene('tell1:이를 이용해 팔라딘 이모젠 라이프는 드레비스의 함선을 밤베르가로 꾀어냈고', 850)
    .addScene('tell1:라이프의 계획은 드레비스의 함선이 밤베르가와 충돌하면서 성공하게 된다.', 850)
    .addScene('tell1:결국 드레비스와 드레비스의 고위 서비터들은 리프 함대에 의해 체포당하게 된다', 850)
    .addScene('tell1:하지만 스콜라스는 자신의 최측근인 드레비스를 구출하고자 했고', 850)
    .addScene('tell1: 피르시스를 보내 이모젠 라이프의 함대를 공격하게 한다', 850)
    .addScene('tell1:', 850)
    .addScene('tell1:', 850)
    .addScene('tell1:', 850)
    .addScene('tell1:', 850)
